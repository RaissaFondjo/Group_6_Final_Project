...
var DistrictsLink = "static/data/Boston_Neighborhoods.geojson";
...
d3.json(DistrictsLink, function(data) {  
  console.log(data);
...
function createFeatures(DistrictData) {

  // Define a function we want to run once for each feature in the features array
  // Give each feature a popup describing the place and time of the earthquake
  function onEachFeature(features, layer) {
    layer.bindPopup("<h2>Neighborhood ID: " + features.properties.Name + "</h2>");
  }

  // Create a GeoJSON layer containing the features array on the earthquakeData object
  // Run the onEachFeature function once for each piece of data in the array
  var Districts = L.geoJSON(DistrictData, {
...
d3.json(fwifiLink, function(data) {
  console.log(data);
  // Creating a geoJSON layer with the retrieved data
 L.geoJson(data, {
    onEachFeature: function(features, layer){
      layer.bindPopup("<h3>" + features.properties.Address + "</h3> <hr> <h4>WIFi: " + features.properties.AP_Name + "</h4>")
    }
  }).addTo(wifi);
